  ...
  return (
    ...
    >
      ...
      <div className="absolute left-3 bottom-3 text-sm text-slate-200 bg-slate-900/60 px-3 py-1 rounded-full space-x-2 flex items-center">
        ...
        <span>
          {frame
            ? `${frame.fps.toFixed(1)} infer${typeof frame.stream_fps === 'number' ? ` / ${frame.stream_fps.toFixed(1)} stream` : ''} fps / ${frame.persons.length} people${typeof frame.latency_ms === 'number' ? ` / ${frame.latency_ms.toFixed(0)} ms` : ''}`
            : 'Buffering…'}
        </span>
