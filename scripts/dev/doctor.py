...
def check_project_files(root: Path) -> list[CheckResult]:
    ...
    if cfg.exists():
        ...
        results.append(CheckResult("config/backend.config.yml", False, "missing (and no example found)"))

    web_pkg = root / "web" / "package.json"
    results.append(CheckResult("web/package.json", web_pkg.exists(), "present" if web_pkg.exists() else "missing"))
    ...
    results.append(CheckResult("testdata/videos", videos.exists(), "present" if videos.exists() else "missing"))

    return results


def check_ports() -> list[CheckResult]:
    # Defaults used by Makefile/README.
...
def main() -> int:
    ...
    if demo_video.exists():
        print("\nDemo (file source):")
        print("- Set video_source=file and video_path=testdata/videos/855564-hd_1920_1080_24fps.mp4 in config/backend.config.yml")
